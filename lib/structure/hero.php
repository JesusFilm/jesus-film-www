...
function hero_title() {
	if ( \class_exists( 'WooCommerce' ) && \is_shop() ) {
		...
		$title = \apply_filters( 'genesis_latest_posts_title', esc_html__( 'Latest Posts', 'jesus-film-project' ) );

	} elseif ( \is_404() ) {
		$title = \apply_filters( 'genesis_404_entry_title', esc_html__( 'Not found, error 404', 'jesus-film-project' ) );

	} elseif ( \is_search() ) {
		$title = \apply_filters( 'genesis_search_title_text', esc_html__( 'Search results for: ', 'jesus-film-project' ) . \get_search_query() );
...
function hero_excerpt() {
	...
	if ( \class_exists( 'WooCommerce' ) && \is_shop() ) {
		...
		$excerpt = \apply_filters( 'genesis_latest_posts_subtitle', esc_html__( 'Showing the latest posts', 'jesus-film-project' ) );
